...
function renderAnalysis(text) {
  ...
  sections.forEach((sec, i) => {
    ...
    if (startIdx === -1) startIdx = text.indexOf(sec.key.replace(/\*\*/g, ''));
    ...
    if (nextSec) {
      ...
      if (ni === -1) ni = text.indexOf(nextSec.key.replace(/\*\*/g, ''), startIdx + 10);
      if (ni !== -1) endIdx = ni;
    }

    let body = text.slice(startIdx + sec.key.length, endIdx).trim()
      .replace(/\*\*/g, '');
