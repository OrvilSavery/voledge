...
═══════════════════════════════
You MUST start each section with the EXACT header shown below, including the ** bold markers.
Do NOT use ##, ###, or any other markdown. Use ONLY the ** format shown.
Do NOT number your output differently. Use exactly these headers in exactly this order.

**1. GO / NO-GO**
One sentence verdict. Flag if IV Rank below 50%, earnings inside window, or no 0.15–0.30 delta strikes exist.

**2. STRATEGY**
Structure choice (iron condor / bull put spread / bear call spread) and why. One sentence on directional bias from price vs S/R and 200MA.

**3. RECOMMENDED STRIKES**
Table: Leg | Strike | Type | Action | Delta | % OTM | vs Expected Move | vs S/R
SHORT legs must have delta 0.15–0.30. If none available, say so and adjust structure.

**4. TRADE METRICS**
Net Credit | Max Profit | Max Loss | Plain English risk statement | Risk/Reward Label | Breakeven Down | Breakeven Up | Credit % of Wing | PoP

**5. BREACH PROBABILITY ANALYSIS**
For each short strike: delta probability, distance from expected move boundary, one-sentence catalyst risk.

**6. RISK FLAGS**
Exactly 3 rows. Format: Risk | Trigger Price | Action

**7. FINAL RECOMMENDATION**
Two sentences: (1) full trade entry with legs/strikes/expiry/min credit, (2) exit plan with profit target and stop price.`;
}
...
function allVariants(key) {
  ...
  return [key, plain, '## ' + plain, '### ' + plain, '#### ' + plain];
}
function findSectionStart(text, key, after) {
  after = after || 0;
  const variants = allVariants(key);
